...
import cn.edu.zzti.soft.util.DialogUtils;
import cn.edu.zzti.soft.util.OrderUtil;
...
public class PayMainActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.paymain_activity);
		initView();
	}
	...
	private void initView() {
		...
		v.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View v) {
				Intent intent = new Intent(PayMainActivity.this, OrderListActivity.class);
	...
	private void pay() {
		int yuan = getPayYuan();
		if (yuan <= 0) {
			Toast.makeText(PayMainActivity.this, R.string.main_topay_fail, Toast.LENGTH_SHORT).show();
			return;
		}
		...
		payApi.pay(order, new OnPayListener() {
			@Override
			public boolean onWillPay(String ticketId, Order order, MobPayAPI api) {
				// 保存ticketId
				return false;
			}

			@Override
			public void onPayEnd(PayResult payResult, Order order, MobPayAPI api) {
				PayMainActivity.this.onPayEnd(order, payResult);
			}
	...
	private void onPayEnd(Order order, PayResult result) {
		if (PayResult.PAYRESULT_OK == result) {
			Toast.makeText(PayMainActivity.this, R.string.main_pay_success, Toast.LENGTH_SHORT).show();
			Intent intent = new Intent(PayMainActivity.this,UnlockActivity.class);
			...
			Toast.makeText(PayMainActivity.this, R.string.main_pay_cancel, Toast.LENGTH_SHORT).show();
		} else {
			Toast.makeText(PayMainActivity.this, R.string.main_pay_fail, Toast.LENGTH_SHORT).show();
